use std::io::{BufRead, BufReader, Seek, SeekFrom, Write, self};
...
impl Log {
    ...
    pub fn new(path: &Path) -> Result<Log, InkError> {
        if path.exists() { return Err(InkError::Err("Path already exists!")) }
    ...
    pub fn serialize(path: &Path) -> Result<Log, InkError> {
        ...
        if let Some(current) = &current_commit {
            ...
            let parsed_current: usize = current.as_ref().unwrap()
                                               .parse().map_err(|_| InkError::Malformed("Log file has invalid commit IDs!"))?;

            let commit_vec: Vec<usize> = commit_vec.iter()
    ...
    pub fn flush(&mut self) -> Result<(), InkError> {
        ...
        if let (Some(current), Some(commits)) = (&self.current_commit, &self.commits) {
            ...
            return Err(InkError::Malformed("Only current or latest commit present in log struct"))
        }
